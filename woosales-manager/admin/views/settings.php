...
if( isset($_POST['wsm_save_settings']) && check_admin_referer('wsm_save_settings') && current_user_can('manage_woocommerce') ){
    ...
    $settings['update_json_url'] = esc_url_raw($_POST['update_json_url'] ?? '');
...
<form method="post">
    ...
    <table class="form-table">
        <tr>
            <th><?php esc_html_e('Provision base','woo-sales-manager'); ?></th>
            <td>
        ...
        <tr>
            <th><?php esc_html_e('Update JSON URL','woo-sales-manager'); ?></th>
            <td>
                <input name="update_json_url" type="url" class="regular-text" value="<?php echo esc_attr($settings['update_json_url']); ?>"/>
                <p class="description"><?php esc_html_e('Example: raw GitHub URL to update.json (like in WP Notification Settings).','woo-sales-manager'); ?></p>
            </td>
        </tr>
    </table>
